#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# Auto-restart wrapper around run_docker.sh, with idle-log watchdog — MI355.
#
...
CONTAINER_NAME="${CONTAINER_NAME:-gpt_oss_120b_eagle3_vllm_mi355}"

LOG_DIR="${REPO_ROOT}/output/GPT_OSS_120b_SDDD/LumenRL"
LOG_FILE="${LOG_DIR}/gpt-oss-120b-eagle3-vllm-mi355.log"
...
while [ "${attempt}" -lt "${MAX_ATTEMPTS}" ]; do
    ...
    if [ -f "${LOG_FILE}" ]; then
        rotated="${LOG_DIR}/gpt-oss-120b-eagle3-vllm-mi355.attempt-$(printf '%02d' "${attempt}").log"
