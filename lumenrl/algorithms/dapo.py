...
class DAPOAlgorithm(BaseAlgorithm):
    ...
    def compute_advantages(self, batch: DataProto) -> DataProto:
        """Inline DAPO advantage computation (dev/OPD fallback path)."""
        ...
        logger.info(
            "DAPO advantages: active_frac=%.4f, adv min=%.4f max=%.4f mean=%.4f, "
            "rewards min=%.4f max=%.4f mean=%.4f, std min=%.6f max=%.6f",
